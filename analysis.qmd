...
filtered_data <- cleaned_data %>% 
  filter(hallucyfq <= 365)

boxplot_hallucinogens_use <- ggplot(filtered_data, aes(x = factor(NEWRACE2), y = hallucyfq, fill = factor(NEWRACE2))) +
  geom_boxplot() +
  labs(title = "TOTAL # OF DAYS USED HALLUCINOGENS IN PAST 12 MOS by RACE",
       x = "Race",
       y = "Total Days of Hallucinogens Use") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 

print(boxplot_hallucinogens_use)
```
> To be or not to be.
```
