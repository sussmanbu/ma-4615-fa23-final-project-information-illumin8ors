...
cleaned_data |>
  filter(cocage < 900) |>
  group_by(NEWRACE2) |>
  ggplot(aes(x = NEWRACE2, y = cocage)) + geom_boxplot() + facet_wrap(vars(COUTYP4),   labeller = labeller(COUTYP4 = facet.labs), scales = "free_y") + theme(axis.text.x = element_text(angle = 90, size = 7)) + labs(title = "Cocaine Use by Race and County Metro Status", x = "Race Category", y = "Estimated Alcohol Use in Past 30 Days") + theme(axis.title.y = element_text(size = 8)) 

cleaned_data |>
  filter(crkage < 900) |>
  group_by(NEWRACE2, COUTYP4) |>
  ggplot(aes(x = NEWRACE2, y = crkage)) + geom_boxplot() + facet_wrap(vars(COUTYP4),   labeller = labeller(COUTYP4 = facet.labs), scales = "free_y") + theme(axis.text.x = element_text(angle = 90, size = 7)) + labs(title = "Crack Use by Race and County Metro Status", x = "Race Category", y = "Estimated Alcohol Use in Past 30 Days") + theme(axis.title.y = element_text(size = 8)) 

cleaned_data |>
  filter(herage < 900) |>
  group_by(NEWRACE2, COUTYP4) |>
  ggplot(aes(x = NEWRACE2, y = herage)) + geom_boxplot() + facet_wrap(vars(COUTYP4),   labeller = labeller(COUTYP4 = facet.labs), scales = "free_y") + theme(axis.text.x = element_text(angle = 90, size = 7)) + labs(title = "Heroine Use by Race and County Metro Status", x = "Race Category", y = "Estimated Alcohol Use in Past 30 Days") + theme(axis.title.y = element_text(size = 8)) 
