...
## Setting the Scene - Maps of How Respondents Are Sorted Across the US According to County Metropolitan Size Status
...
subtitle_text <- "1 = County in CBSA with 1 Million Plus Residents, 2 = County in CBSA with Less Than 1 Million Residents,\n3 = County not in a CBSA"

pop_map <- counties_cleaned_POP2020 |> filter(STUSPS != "AK", STUSPS != "HI", STUSPS != "MP", STUSPS != "AS", STUSPS != "PR", STUSPS != "GU", STUSPS != "VI") %>% ggplot() + geom_sf(aes(fill = popstatus), alpha = 0.5) + scale_fill_viridis_c() + ggtitle("US Counties Colored by Population Status") + labs(subtitle = subtitle_text, fill = "Population Status") + theme(plot.subtitle = element_text(size = 9))
...
cleaned_data |>
  filter(ccdaypwk < 85) |>
  group_by(NEWRACE2) |>
  ggplot(aes(x = NEWRACE2, y = ccdaypwk)) + geom_boxplot() + facet_wrap(vars(COUTYP4),   labeller = labeller(COUTYP4 = facet.labs), scales = "free_y") + theme(axis.text.x = element_text(angle = 90, size = 7)) + labs(title = "Days per Week of Cocaine Use by Race and County Metro Status", x = "Race Category", y = "Age") + theme(axis.title.y = element_text(size = 9)) 

cleaned_data |>
  filter(crdaypwk < 85) |>
  group_by(NEWRACE2, COUTYP4) |>
  ggplot(aes(x = NEWRACE2, y = crdaypwk)) + geom_boxplot() + facet_wrap(vars(COUTYP4),   labeller = labeller(COUTYP4 = facet.labs), scales = "free_y") + theme(axis.text.x = element_text(angle = 90, size = 7)) + labs(title = "Days per Week of Crack Use by Race and County Metro Status", x = "Race Category", y = "Age") + theme(axis.title.y = element_text(size = 9)) 

cleaned_data |>
  filter(hrdaypwk < 85) |>
  group_by(NEWRACE2, COUTYP4) |>
  ggplot(aes(x = NEWRACE2, y = hrdaypwk)) + geom_boxplot() + facet_wrap(vars(COUTYP4),   labeller = labeller(COUTYP4 = facet.labs), scales = "free_y") + theme(axis.text.x = element_text(angle = 90, size = 7)) + labs(title = "Days per Week of Heroin Use by Race and County Metro Status", x = "Race Category", y = "Age") + theme(axis.title.y = element_text(size = 9)) 
...
ggplot(her_data, aes(x = NEWRACE2, y = herage, fill = NEWRACE2)) +
  geom_boxplot() +
  labs(title = "Boxplot of Age of First Drug Use of Heroin by Race",
       x = "Race",
       y = "Age of First Use",
       fill = "Race Type") +   
  theme_minimal() +
...
We assume for the model here that the hypothesis is that there is no association between likelihood of trying drugs ever and race group as well as Income.
...
Same as before, we assume for the model here that the hypothesis is that there is no association between likelihood of trying drugs ever and race group as well as Income.
```{r}
