...
While the data set we are using from the 2021 NSDUH contains bountiful information concerning substance use, race, income, mental health, education, and age, it does not include very specific location information. The main information provided about the location of the survey respondents is stored in the variables "PDEN10" and "COUTYP4". The PDEN10 variable represents "Population Density 2010" and respondents are sorted into coming from county based statistical areas (CBSAs) of 1 million or more people, less than 1 million people, or as not in a CBSA. The NSDUH survey designers created those classifications based on 2013 CBSA codes and 2010 Census data. They used the Census data to obtain the population counts for the different CBSA or non-CBSA areas. Although it was not available by the survey makers at the time, we decided to use 2020 Census data to provide a more up-to-date visualization of the populations of the three categories of the PDEN10 variable. This more closely reflects the county statuses at the time the survey data was collected in 2021. The COUNTYP4 variable sorts respondents based on rural urban continuum codes (RUCC) from 2013. It places respondents into large metro, small metro, or non-metro counties. To visualize these counties on a US map, we downloaded and cleaned an excel data file of the 2013 RUCC.
...
For the location analysis portion of our project, we used data from the 2020 Census, the 2009 CBSA, and the 2013 Rural Urban Continuum Codes (RUCC). To clean the CBSA data set, the readxl package was used to read the CBSA excel file. The data was already pretty clean, but in the original file there was a blank row and a few header rows, so we had to make sure to skip those. Then the column names were labeled and any missing values were converted to read as NA. A similar procedure was followed to read in an excel file containing Census data. With the Census data, we were interested in county populations by state. 

One thing we had to consider with the Census data was that it contained a row at the start of each state where the county name and the state name were the same and this row listed the state’s total population. While we did not remove this row when cleaning the data set, we were aware of it and would filter it out if necessary. For example, it was not necessary to filter out this row when we created a merged CBSA/CENSUS dataset since we joined the datasets on the conditions that a pair of state and county names matched in both datasets and since the CBSA data did not have any such pairs, it would automatically be eliminated. This merged dataset would provide insight into the “PDEN10”, or Population Density 2010, variable, but we did not end up exploring that much further because we became more interested in how the metropolitan status of a county was related to drug use and this insight was provided by combining Census, CBSA, and RUCC data. 

We ended up using the merged Census/CBSA data to find overlap between the Census and RUCC counties not only to stay true to the techniques of the survey designers, but also because the CBSA data provided FIPS codes of counties in the Census, and in the RUCC dataset counties were identified by their FIPS codes so this made the overlap easy. We then used this 3-way merged data set to create the map visualizing county metro status in the US. 

We also had to download and use the 'tigris' package to make this map because we needed the shape files/geometries of the counties of interest. Tigris is an R package that grants you access to the shape files of counties or states based on US Census data. We took advantage of the counties() function within tigris to pull county geometries and then combined them with lists of large, small, and non-metro counties we had made based on the Census, CBSA, and RUCC data sets. With that information, we were able to make a US map that shaded the US counties by metro status. 

One important thing that we had to look into when cleaning the CBSA, CENSUS, and RUCC data sets was the column types since they were all excel files. We tried using the character() function to tell us the types of columns we were dealing with to pass into the read_excel() function. However, we still ran into parsing errors because the excel files had numeric values stored as texts. To address this issue, we used a feature of read_excel() that allowed us to pass in a column type called “guess”, allowing R to guess at/decide on how it wanted to read in the numeric values stored as text which fixed the error message and produced readable data. Once this error had been fixed, we were able to save the cleaned CBSA, Census, and RUCC data sets as RDS files.
